...
#' @param ILMN_outfile 输出特定探针的smr结果
#' @param ILMN_ID 输入特定探针号
#' @param smr_thresh SMR的阈值
#' @param trait_name 结局名称
#' @param num 计算机启动线程最大数，默认10
#' @param LocusPlot 区域GWAS图
...
SMR_plot<-function(pop="EUR",maGWASfile,eqtlfile,ILMN_outfile,ILMN_ID,smr_thresh,trait_name,num,LocusPlot=F,EffectPlot=T){
  ...
  library("TeachingDemos")
    if(Sys.info()["sysname"] == "Windows"){
      ...
      shell1<-paste0(getwd(),"/SMR/smr_Mac/","smr-1.3.1-macos-arm64 --bfile ",getwd(),"/1kg.v3/",pop," --gwas-summary ",maGWASfile," --beqtl-summary ",eqtlfile," --out ",ILMN_outfile," --plot --probe ",ILMN_ID," --probe-wind 500 ","--gene-list ",getwd(),"/SMR/glist-hg19"," --thread-num ",num," --diff-freq--prop 0.99")
      system(shell1)}
