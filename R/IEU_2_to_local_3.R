...
#' @param data2IV 来自IEU暴露2工具变量文件
#' @param GWASID2 来自IEU暴露2的GWAS ID号
...
#' @export
IEU_2_to_local_3<-function(data1IV,GWASID1,data2IV,GWASID2,data3IV,data3GWAS,data4IV,data4GWAS,data5IV,data5GWAS){
  library(tidyr)
    ...
    data_h_SNP_steiger_mv5_1<-extract_outcome_data(snps=data5IV$SNP,outcomes=GWASID1,proxies=T,maf_threshold = 0.01)
    data_h_SNP_steiger_mv5_1<-data_h_SNP_steiger_mv5_1[,out_name]
    colnames(data_h_SNP_steiger_mv5_1)<-exp_name
    data_h_SNP_steiger_mv5_2<-extract_outcome_data(snps=data5IV$SNP,outcomes=GWASID2,proxies=T,maf_threshold = 0.01)
    ...
    MVIV3_5<-data_h_SNP_steiger_mv5_3[,exp_name]#EXP4在自己GWAS中的IVs
    ...
    MVIV4_5<-data_h_SNP_steiger_mv5_4[,exp_name]#EXP4在自己GWAS中的IVs
