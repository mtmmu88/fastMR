#' @title 组学孟德尔随机化
#' @param fac_cell_met_gut 选择运行那种组学,1,2,3,4代表炎症因子,免疫细胞,代谢
#' @param savefile 结果输出保存的文件夹
#' @param omicfile 输入组学数据存放的文件夹
#' @param finish_exp 输入暴露的GWAS summary
...
#' @export
local_Omic<-function(fac_cell_met_gut=1,savefile="MR结果",omicfile,finish_exp,local_clump=F,clump_p1=1e-05,clump_r2=0.001,clump_kb=10000,pop="EUR",presso=F){
...
expiv0<-expiv0%>%filter(!SNP%in%NA)
  ...
  if (ask_user1()) {
  expiv<- local_clump_data1(expiv0,clump_kb = clump_kb,clump_r2 = clump_r2,pop = pop)}else {stop("请更换更大的clump_p1")}

  ask_user2 <- function() {
    answer <- readline(prompt=message("你当前暴露的工具变量满足所有条件的","有",dim(expiv)[[1]],"个是否继续运行反向组学MR?"))
    return(tolower(answer) == "是")
  }
  if (ask_user2()) {
if(class(expiv$eaf.exposure[1])!="logical"){
    ...
    for (id in file5[,1]){
      ...
      OUT<-fread(exppath,header=T)%>%data.frame()
      ...
      if(dim(total1)[[1]]!=0){
        ...
        if(class(test1)!="try-error"&dim(data_h_F10)[[1]]!=0){
          ...
          if(dim(data_h_F10_steiger)[[1]]!=0){
            ...
            res$F10<-1
            res$id<-id
            res$IV<-paste0("stand is ","P<",clump_p1,"kb=",clump_kb,"r2=",clump_r2)
            ...
            data_h_TableS1$steiger<-1
            data_h_TableS1$F10<-1
            data_h_TableS1$file5<-id
            A_temp <- rbind(mr_OR, A_temp)
            B_temp <- rbind(het, B_temp)
            C_temp <- rbind(ple, C_temp)
            D_temp <- rbind(data_h_TableS1, D_temp)
            write.csv(A_temp, paste0(getwd(),"/",savefile,"/","MR.csv"), row.names = F)
            write.csv(B_temp, paste0(getwd(),"/",savefile,"/","het.csv"), row.names = F)
            write.csv(C_temp, paste0(getwd(),"/",savefile,"/","ple.csv"), row.names = F)
            write.csv(D_temp, paste0(getwd(),"/",savefile,"/","IV.csv"), row.names = F) }else{H_temp1 <- rbind(H_temp1, id)%>%data.frame()
            H_temp1$reason<-paste0("因不满足反向过滤被排除的",nm)
            write.csv(H_temp1, paste0(getwd(),"/",savefile,"/","NOsteigerid.csv"), row.names = F)}}else{H_temp2 <- rbind(H_temp2, id)%>%data.frame()
            H_temp2$reason<-paste0("因不满足回文等被排除的",nm)
            write.csv(H_temp2, paste0(getwd(),"/",savefile,"/","NOclumpid.csv"), row.names = F)}}else{H_temp3 <- rbind(H_temp3, id)%>%data.frame()
            H_temp3$reason<-paste0("因无法匹配到工具变量被排除的",nm)
            write.csv(H_temp3, paste0(getwd(),"/",savefile,"/","NOmergeid.csv"), row.names = F)}
      print(id)
      row_numbers <- which(file5$file5 == id)
      pv<-round((row_numbers /N)*100,3)
      cat("已完成",pv,"%")
    }
}else {stop("请更换更大的clump_p1")}
} else{
    library(MRPRESSO)
    #多效性偏差
